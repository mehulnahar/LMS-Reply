...
// ════════════════════════════════════════════════════════════
// TEMP: Bulk test endpoints for research pipeline validation
// ════════════════════════════════════════════════════════════

// List all emails with matched jobs (metadata only, no research)
router.post('/debug-list', async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT e.id, e.from_email, e.subject, j.job_heading
       FROM emails e
       JOIN jobs j ON j.email_id = e.id
       WHERE e.user_id = $1 AND j.match_status = 'matched'
...
// Test research on a single email by ID
router.post('/debug-test', async (req, res, next) => {
  try {
    ...
    // Look up email + job context (same logic as main route)
    const { rows } = await pool.query(
    ...
    // Build enriched description (same logic as main route)
    let enrichedDescription = '';
    const emailBody = r.body_text || r.snippet || '';
    if (r.job_heading) {
      enrichedDescription = r.job_heading;
      if (r.job_description) enrichedDescription += ' | ' + r.job_description.substring(0, 1000);
    } else {
      enrichedDescription = r.subject || 'General project';
    }
    let fullContext = enrichedDescription;
    if (emailBody) fullContext += '\n\nClient email: ' + emailBody.substring(0, 500);

    const [a, e, o] = await Promise.all([getApiKey(userId, 'anthropic'), getApiKey(userId, 'exa'), getApiKey(userId, 'olostep')]);
    const result = await researchSimilarExamples(fullContext, a, e, o);

    res.json({
      email: { from: r.from_email, subject: r.subject, jobHeading: r.job_heading },
      exampleCount: result.examples.length,
      examples: result.examples.map(ex => ({ name: ex.name, url: ex.url })),
      queries: result.debug ? result.debug.searchQueries : [],
      rawResultCount: result.rawResultCount,
      scrapedCount: result.scrapedCount,
    });
  } catch (err) { next(err); }
